...
func GetOptimalClientSubnet() string {
	...
	if err != nil {
		...
		if subnet := model.GetDefaultClientSubnet(); subnet != "" {
			return subnet
		}
...
func getRealLocation() (*model.LocationInfo, error) {
	// 使用多个IP查询服务，提高成功率
	services := []string{
		"https://ipapi.co/json/",
		"http://ip.sb/geoip/",
...
func queryLocationService(client *http.Client, serviceURL string) (*model.LocationInfo, error) {
...
func parseIPAPIResponse(body []byte) (*model.LocationInfo, error) {
	var response struct {
	...
	normalizedISP = model.NormalizeISPName(response.ISP)
...
func parseIPAPICoResponse(body []byte) (*model.LocationInfo, error) {
	var response struct {
		IP      string `json:"ip"`
	...
	cityName = model.GetCityNameCH(response.City)
	normalizedISP = model.NormalizeISPName(response.Org)
...
func parseIPSBResponse(body []byte) (*model.LocationInfo, error) {
	var response struct {
		IP           string `json:"ip"`
		Country      string `json:"country"`
		Region       string `json:"region"`
	...
	var normalizedISP string
	normalizedISP = model.NormalizeISPName(isp)
