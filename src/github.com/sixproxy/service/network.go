...
// CityMappingProvider 城市映射功能提供者接口
type CityMappingProvider interface {
	GetDefaultClientSubnet() string
	GetCityISPSubnet(city, isp string) string
	GetFallbackSubnet(location *LocationInfo) string
	InferCityFromRegion(region string) string
	GetCityNameCH(cityName string) string
	GetDefaultCityByISP(isp string) string
	NormalizeISPName(isp string) string
}

var cityMappingProvider CityMappingProvider

// SetCityMappingProvider 设置城市映射提供者
func SetCityMappingProvider(provider CityMappingProvider) {
	cityMappingProvider = provider
}
...
func GetOptimalClientSubnet() string {
	...
	if err != nil {
		...
		if cityMappingProvider != nil {
			return cityMappingProvider.GetDefaultClientSubnet()
		}
		return "1.2.4.0/24" // 默认值
	}

	// 2. 根据城市和运营商获取精确的网段
	if cityMappingProvider != nil {
		if subnet := cityMappingProvider.GetCityISPSubnet(location.City, location.ISP); subnet != "" {
			logger.NetworkInfo("检测到位置: %s %s，使用client_subnet: %s", location.City, location.ISP, subnet)
			return subnet
		}

		// 3. 如果精确匹配失败，尝试模糊匹配
		if subnet := cityMappingProvider.GetFallbackSubnet(location); subnet != "" {
	...
	for _, service := range services {
		...
		if err == nil && location != nil {
			...
			if originalCity == "" {
				...
				// 尝试从省份/地区推导城市
				if location.Region != "" && cityMappingProvider != nil {
					originalCity = cityMappingProvider.InferCityFromRegion(location.Region)
			...
			if cityMappingProvider != nil {
				mappedCity = cityMappingProvider.GetCityNameCH(originalCity)
			}
			...
			if mappedCity != "" && mappedCity != originalCity {
				...
				if cityMappingProvider != nil {
					location.City = cityMappingProvider.GetDefaultCityByISP(location.ISP)
				} else {
					location.City = "北京" // 默认值
				}
			}
...
func parseIPAPIResponse(body []byte) (*LocationInfo, error) {
	...
	var normalizedISP string
	if cityMappingProvider != nil {
		normalizedISP = cityMappingProvider.NormalizeISPName(response.ISP)
	} else {
		normalizedISP = response.ISP
	}
...
func parseIPAPICoResponse(body []byte) (*LocationInfo, error) {
	...
	var cityName, normalizedISP string
	if cityMappingProvider != nil {
		cityName = cityMappingProvider.GetCityNameCH(response.City)
		normalizedISP = cityMappingProvider.NormalizeISPName(response.Org)
	} else {
		cityName = response.City
		normalizedISP = response.Org
	}
...
func parseIPSBResponse(body []byte) (*LocationInfo, error) {
	...
	var normalizedISP string
	if cityMappingProvider != nil {
		normalizedISP = cityMappingProvider.NormalizeISPName(isp)
	} else {
		normalizedISP = isp
	}
